...
Based on Rochet2's Transmogrification 3.3.5a - Gossip menu
Adapted for slot-based transmogrification
...
struct PendingItemsCheck : public BasicEvent
{
    ...
    bool Execute(uint64, uint32) override
    {
        ...
        // Revert pending visuals back to saved state using slot-based UpdateSlotVisual
        for (auto& pending : pending_transmogs)
            Transmogrification::instance().UpdateSlotVisual(player, std::get<0>(pending));
        return true;
    }
                ...
                {
                    {
                        std::string status;
                        uint32 pending;
                        // Slot-based: read transmog entry from player slot data
                        uint32 entry = player->GetSlotTransmog(slot).transmogEntry;
                        bool hasPending = Transmogrification::instance().HasPendingTransmog(player, slot, &pending);
                        bool hasTransmog = entry != 0;
                        if (hasTransmog)
                            status += " [transmog]";
                        if (hasPending)
                        {
                    ...
                    for (uint32 slotindex = 0; slotindex < ENCHANT_SLOTS.size(); ++slotindex)
                    {
                        if (slot == ENCHANT_SLOTS[slotindex])
                        {
                            std::string status;
                            uint32 pending;
                            // Slot-based: read enchant entry from player slot data
                            uint32 entry = player->GetSlotTransmog(slot).enchantEntry;
                            bool hasPending = Transmogrification::instance().HasPendingEnchant(player, slot, &pending);
                            bool hasTransmog = entry != 0;
                            if (hasTransmog)
                                status += " [enchant]";
                            if (hasPending)
