...
public:
    ...
    static const std::unordered_map<uint32, std::string> enchant_visual_to_name;
    ...
    std::unordered_map<uint32, std::vector<SpellInfo const*>> enchant_to_spells;
    std::unordered_map<uint32, std::vector<uint32>> spell_to_enchants;
    ...
    void UpdateSlotVisual(Player* player, uint8 slot);
    ...
    bool HasPendingTransmog(Player* player, uint8 slot, uint32* retPending = nullptr, uint32* retCurrent = nullptr);
    bool HasPendingEnchant(Player* player, uint8 slot, uint32* retPending = nullptr, uint32* retCurrent = nullptr);
    ...
    uint32 GetCurrentVisual(Player* player, uint8 slot);
    uint32 GetCurrentVisualEnchant(Player* player, uint8 slot);
    ...
    void Transmogrify(Player* player, uint8 slot, AppearanceType type, uint32 entry);
