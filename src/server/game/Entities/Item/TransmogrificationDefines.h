...
// Slot-based transmog data stored per equipment slot on the player
struct SlotTransmogData
{
    uint32 transmogEntry = 0;
    uint32 enchantEntry = 0;
};

// Slot-based pending transmogs: slot instead of Item*
typedef std::vector<std::tuple<uint8 /*slot*/, uint32 /*transmog*/, AppearanceType /*transmog type*/>> PendingTransmogs;
