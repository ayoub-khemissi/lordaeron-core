...
void Transmogrification::UpdateSlotVisual(Player* player, uint8 slot)
{
    TC_LOG_DEBUG("custom.transmog", "Transmogrification::UpdateSlotVisual");
...
TransmogResult Transmogrification::CannotTransmogrifyItemWithItem(Player* player, ItemTemplate const* target, ItemTemplate const* source, bool ever)
{
    ...
    if (source->SubClass != target->SubClass && !IsBowOrGunOrCrossbow(target))
    {
        ...
        if (source->Class == ITEM_CLASS_WEAPON && !IsMeleeWeapon(target))
            return TransmogResult_WeaponTypesDontMatch;
    }
...
void Transmogrification::LoadEnchants()
{
    ...
    for (auto* enchant : sSpellItemEnchantmentStore) {
        if (enchant && enchant->ItemVisual && enchant->ItemVisual < static_cast<uint32>(-1))
            enchant_to_spells[enchant->ID];
    }
...
{
    TC_LOG_DEBUG("custom.transmog", "Transmogrification::TrySetPendingTransmog");

    if (slot >= EQUIPMENT_SLOT_END)
    {
        TC_LOG_DEBUG("custom.transmog", "Transmogrification::TrySetPendingTransmog - {} ({}) tried to transmogrify invalid slot ({}) with {}.", player->GetName(), player->GetGUID().ToString(), slot, entry);
    ...
    if (hasTemplate)
    {
        ...
        if (!itemtemplate)
        {
            TC_LOG_DEBUG("custom.transmog", "Transmogrification::TrySetPendingTransmog - {} ({}) tried to transmogrify slot {} with a non-existant item entry {}.", player->GetName(), player->GetGUID().ToString(), slot, entry);
            return TransmogResult_NonexistantTransmog;
        }
    }
    ...
    {
        TC_LOG_DEBUG("custom.transmog", "Transmogrification::TrySetPendingTransmog - {} ({}) tried to transmogrify slot {} with entry {}, but the slot was empty.", player->GetName(), player->GetGUID().ToString(), slot, entry);
    ...
    if (hasTemplate)
    {
        ...
        {
            TC_LOG_DEBUG("custom.transmog", "Transmogrification::TrySetPendingTransmog - {} ({}) failed CannotTransmogrifyItemWithItem ({} with {}).", player->GetName(), player->GetGUID().ToString(), itemTransmogrified->GetEntry(), entry);
            return res;
        }
    }

    if (entry == InvisibleEntry && (slot == EQUIPMENT_SLOT_MAINHAND || slot == EQUIPMENT_SLOT_OFFHAND || slot == EQUIPMENT_SLOT_RANGED))
        return TransmogResult_NonexistantTransmog;
...
{
    TC_LOG_DEBUG("custom.transmog", "Transmogrification::TrySetPendingEnchant");

    if (slot >= EQUIPMENT_SLOT_END)
    {
        TC_LOG_DEBUG("custom.transmog", "Transmogrification::TrySetPendingEnchant - {} ({}) tried to transmogrify invalid slot ({}) with {}.", player->GetName(), player->GetGUID().ToString(), slot, entry);
        return TransmogResult_InvalidSlot;
    }
    ...
    if (!itemTransmogrified)
    {
        TC_LOG_DEBUG("custom.transmog", "Transmogrification::TrySetPendingEnchant - {} ({}) tried to transmogrify slot {} with entry {}, but the slot was empty.", player->GetName(), player->GetGUID().ToString(), slot, entry);
        return TransmogResult_EmptySlot;
    }
    ...
    if (hasTemplate)
    {
        if (TransmogResult res = CannotTransmogrifyItemWithEnchant(player, itemTransmogrified->GetTemplate(), entry, false)) {
            TC_LOG_DEBUG("custom.transmog", "Transmogrification::TrySetPendingEnchant - {} ({}) failed CannotTransmogrifyItemWithEnchant ({} with {}).", player->GetName(), player->GetGUID().ToString(), itemTransmogrified->GetEntry(), entry);
            return res;
        }
    }
    ...
    // Set pending enchant visually
    player->SetUInt16Value(PLAYER_VISIBLE_ITEM_1_ENCHANTMENT + (slot * 2), 0, entry);
    return TransmogResult_Ok;
}

void Transmogrification::Transmogrify(Player* player, uint8 slot, AppearanceType type, uint32 entry)
{
    // SLOT-BASED: write to player's slot data instead of item
    if (entry == InvisibleEntry)
    ...
    switch (type)
    {
        ...
        {
            SlotTransmogData data = player->GetSlotTransmog(slot);
            data.transmogEntry = entry;
            player->SetSlotTransmog(slot, data.transmogEntry, data.enchantEntry);
            break;
        }
        case TRANSMOG_TYPE_ENCHANT:
        {
            SlotTransmogData data = player->GetSlotTransmog(slot);
            data.enchantEntry = entry;
            player->SetSlotTransmog(slot, data.transmogEntry, data.enchantEntry);
            break;
        }
...
{
    // Slot-based: clear all slot transmog data and restore visuals
    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        player->ClearSlotTransmog(slot);
        Item* item = GetEquippedItem(player, slot);
        if (item)
        {
...
{
    // Revert pending (unsaved) visual changes back to current saved state
    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        Item* item = GetEquippedItem(player, slot);
        if (item)
...
{
    uint32 pending;
    if (HasPendingTransmog(player, slot, &pending))
    {
...
{
    uint32 pending;
    if (HasPendingEnchant(player, slot, &pending))
    {
...
bool Transmogrification::HasPendingTransmog(Player* player, uint8 slot, uint32* retPending, uint32* retCurrent)
{
    Item* item = GetEquippedItem(player, slot);
    if (!item)
        return false;
    uint32 current = GetCurrentVisual(player, slot);
    uint32 pending = player->GetUInt32Value(PLAYER_VISIBLE_ITEM_1_ENTRYID + (slot * 2));
    if (current == item->GetEntry())
        current = NormalEntry;
    if (current == 0)
        current = InvisibleEntry;
...
bool Transmogrification::HasPendingEnchant(Player* player, uint8 slot, uint32* retPending, uint32* retCurrent)
{
    Item* item = GetEquippedItem(player, slot);
    if (!item)
        return false;
    uint32 current = GetCurrentVisualEnchant(player, slot);
    uint32 pending = player->GetUInt16Value(PLAYER_VISIBLE_ITEM_1_ENCHANTMENT + (slot * 2), 0);
...
PendingTransmogs Transmogrification::GetPendingTransmogs(Player* player)
{
    ...
    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        uint32 pending;
        if (HasPendingTransmog(player, slot, &pending))
            pending_transmogs.emplace_back(slot, pending, TRANSMOG_TYPE_ITEM);
        if (HasPendingEnchant(player, slot, &pending))
            pending_transmogs.emplace_back(slot, pending, TRANSMOG_TYPE_ENCHANT);
    }
    return pending_transmogs;
}

uint32 Transmogrification::GetCurrentVisual(Player* player, uint8 slot)
{
    // Slot-based: read from player's slot data
    SlotTransmogData const& data = player->GetSlotTransmog(slot);
    if (data.transmogEntry)
        return data.transmogEntry;
    Item* item = GetEquippedItem(player, slot);
    if (item)
...
uint32 Transmogrification::GetCurrentVisualEnchant(Player* player, uint8 slot)
{
    // Slot-based: read from player's slot data
    SlotTransmogData const& data = player->GetSlotTransmog(slot);
    if (data.enchantEntry)
        return data.enchantEntry;
    Item* item = GetEquippedItem(player, slot);
    if (item)
...
TransmogResult Transmogrification::TransmogrifyPending(Player* player, int32 expectedCost /*= -1*/)
{
    ...
    {
        Transmogrify(player, std::get<uint8>(transmog), std::get<AppearanceType>(transmog), std::get<uint32>(transmog));
    }
    return TransmogResult_Ok;
}
